...
public class SortingMethods {
	...
	public static <E extends Comparable<? super E>> void insertSort(E[] arr) {
		...
		// process from 2nd item to end of list, skip first item because already
		// considered sorted
	...
	public static <E extends Comparable<? super E>> void merge(E[] arr, E[] temp, int leftStart, int rightStart,
			int rightEnd) {
	...
	public static <E extends Comparable<? super E>> void quickSort(List<E> list) {
		...
		if (list.size() > 1) {
			...
			// System.out.println("Smaller:"+smaller.size()+" :: Same:"+same.size()+" ::
			// Larger:"+larger.size());
	...
	public static void radixSortDifferentLength(String[] arr) {
		...
		// DO NOT use the version in the textbook, modify the same length to work for
		// different length
		// DO NOT create any additional arrays/data structures, changing the number of
		// buckets is allowed

		int bucketCount = 256; // number of buckets (256 in standard character set)
		int maxLength = 0; // max number of letters in string for padding
